package awa.hyw.Axolotl.util.input
...
data class KeyBind @JvmOverloads constructor(
    ...
    companion object {
        ...
        if (scanCode == 310 || key == GLFW.GLFW_KEY_RIGHT_SHIFT) return "RShift"
        if (key == GLFW.GLFW_KEY_UNKNOWN) {
                for (k in 32..348) {
                    if (GLFW.glfwGetKeyScancode(k) == scanCode) {
                        return getKeyName(k, scanCode)
                    }
                }
                return GLFW.glfwGetKeyName(GLFW.GLFW_KEY_UNKNOWN, scanCode) ?: if (scanCode == 0) "None" else "ScanCode $scanCode"
            }
    ...
    fun valueFromString(string: String) {
        ...
        if (split.size < 3) {
            ...
            if (split.size >= 2) {
                this.type = runCatching { Type.valueOf(split[0]) }.getOrNull() ?: Type.KEYBOARD
                this.keyCode = split.getOrNull(1)?.toIntOrNull() ?: -1
                if (this.keyCode != -1) {
                    this.scanCode = GLFW.glfwGetKeyScancode(this.keyCode)
                }
            }
            return
        }
        this.type = runCatching { Type.valueOf(split[0]) }.getOrNull() ?: Type.KEYBOARD
        this.keyCode = split.getOrNull(1)?.toIntOrNull() ?: -1
        this.scanCode = split.getOrNull(2)?.toIntOrNull() ?: -1
        
        // Fix missing scanCode
        if (this.keyCode != -1 && this.scanCode <= 0) {
            this.scanCode = GLFW.glfwGetKeyScancode(this.keyCode)
        }
    }
