...
import awa.hyw.patchify.annotation.*;
import awa.hyw.patchify.api.ILocals;
import awa.hyw.patchify.api.IPatchLoader;
import awa.hyw.patchify.api.Invocation;
import awa.hyw.patchify.asm.InvocationImpl;
import awa.hyw.patchify.asm.Locals;
...
public final class PatchLoader implements IPatchLoader {
    ...
    private static List<AbstractInsnNode> getInjectionPoints(InsnList insnList, Slice slice,
        ...
        log.info("Finding injection points for slice: start={}, end={}", slice.start().value(), slice.end().value());
        if (slice.start().value() == At.Type.HEAD && slice.end().value() == At.Type.TAIL && slice.startIndex() == -1 && slice.endIndex() == -1) {
            log.info("head-tail slice injection point found!");
            ...
            for (AbstractInsnNode insnNode : insnList) {
                if (filter.test(insnNode)) {
                    count++;
                    log.info("index-based slice match found!, count = {}", count);
                    if (count >= slice.startIndex() && count <= endIndex) {
                        log.info("index-based slice injection point found!");
            ...
            log.info("method/head-tail before/after slice injection point found!");
            ...
            if (mapping != null) {
                if (slice.start().remapped().isEmpty()) {
                    ...
                    if (mapping.revMethodsMapping.containsKey(start)) {
                        ...
                        log.info("Mapped start method: {} -> {}", slice.start().method(), start.first());
                    } else {
                ...
                if (slice.end().remapped().isEmpty()) {
                    ...
                    if (mapping.revMethodsMapping.containsKey(end)) {
                        ...
                        log.info("Mapped end method: {} -> {}", slice.end().method(), end.first());
                    } else {
            ...
            for (AbstractInsnNode insnNode : insnList) {
                ...
                if (foundStart) {
                    log.info("method-based slice injection point found!");
                    injectionPoints.add(insnNode);
                }
            }
        }
    ...
    private static void injectHead(MethodNode method, Method inject) {
        log.info("Injecting HEAD: " + inject.getName());
    ...
    private static void injectTail(MethodNode method, Method inject) {
        log.debug("Injecting TAIL: " + inject.getName());
    ...
    private static void injectMethod(MethodNode method, Method inject, Pair<String, String> invoke0, boolean isBefore) {
        log.debug("Injecting Method: " + inject.getName() + " invoke: " + invoke0.first());
        var returnType = Type.getReturnType(method.desc);
        var injectOwner = Type.getInternalName(inject.getDeclaringClass());
        var injectName = inject.getName();
        var injectDesc = Type.getMethodDescriptor(inject);
    ...
    public Pair<Class<?>, byte[]> patch(Function<Class<?>, byte[]> bytesProvider,
        ...
        for (Method method : patchClass.getDeclaredMethods()) {
            if (method.isAnnotationPresent(Inject.class) || method.isAnnotationPresent(Overwrite.class) ||
                ...
                String className = targetNode.name;
                if (mapping != null && mapping.classesMapping.containsKey(className)) {
                    className = mapping.classesMapping.get(className);
                }
                Pair<String, String> target = Pair.of(className + "/" + name, desc);
                if (mapping != null) {
